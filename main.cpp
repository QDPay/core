...
const string strMessageMagic = "QDPay Signed Message:\n";
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
       const char* pszTimestamp = "already in already databases we QDPaypark from USA with Transactions.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("QDPayMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("QDPayMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("QDPayMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("QDPay-miner");
    ...
    try { loop {
        ...
        printf("Running QDPayMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
